...
async function pingServer() {
  try {
    await axios.get(PING_URL, { timeout: 90000 });
    return true;
  } catch {
    return false;
  }
      ...
      <div className="flex items-center gap-3 bg-amber-50 border border-amber-200 rounded-2xl px-4 py-3 text-amber-700 text-sm">
        ...
        <div>
          ...
          <span className="ml-2 text-amber-600">
            Render free tier sleeps after inactivity. This takes 30–60 seconds.
            Please wait.
          </span>
        </div>
      ...
      <div className="flex items-center gap-3 bg-red-50 border border-red-200 rounded-2xl px-4 py-3 text-red-700 text-sm">
        ...
        <div>
          ...
          <span className="text-red-600">
            Check your internet or try refreshing the page.
          </span>
        </div>
      </div>
    );
...
function App() {
  ...
  // Track whether rows have been animated already (only animate once on first load)
  const [hasAnimated, setHasAnimated] = useState(false);

  // Manual ping state
  const [pinging, setPinging] = useState(false);
  const [pingResult, setPingResult] = useState(null); // null | "success" | "fail"
  ...
  /* manual ping — user clicks the Ping button */
  const manualPing = async () => {
    ...
    // Start counting seconds while pinging
  ...
  const fetchRecords = useCallback(async () => {
    try {
      ...
      setRecords(res.data || []);
      // Mark that we should animate this batch of rows
      setHasAnimated(false);
    } catch {
      toast.error("Failed to load records");
    } finally {
      setLoading(false);
      // After records are set and rendered, flip the flag so animation plays once
  ...
  useEffect(() => {
    ...
    setFiltered(
      karat === "ALL" ? records : records.filter((r) => r.karatType === karat),
    );
    // Re-trigger animation when filter changes (optional — remove if you don't want this)
    setHasAnimated(false);
    setTimeout(() => setHasAnimated(true), 50);
  ...
  const addRecord = async () => {
    ...
    if (serverStatus !== "online") {
      toast.warning("Server is still waking up, please wait…");
      return;
    }
    try {
      await axios.post(
      ...
      toast.error("Error adding record. Try again.");
    }
  };
...
const exportPDF = () => {
  ...
  return (
    <div className={`min-h-screen p-3 md:p-6 space-y-4 ${bg}`}>
      ...
      {/* HEADER */}
      <div
        className={`${card} border rounded-3xl p-4 flex justify-between items-center`}
      >
        <div>
          ...
          <div className="flex items-center gap-2 mt-1 flex-wrap">
            <span
              className={`w-2 h-2 rounded-full flex-shrink-0 ${serverStatus === "online" ? "bg-green-500 animate-pulse" : serverStatus === "waking" ? "bg-amber-400 animate-pulse" : "bg-red-500"}`}
            />
            <span className="text-xs text-slate-400">
              {serverStatus === "online"
                ? "Server online"
                : serverStatus === "waking"
                  ? `Waking up server… ${wakeSeconds}s`
                  : "Server offline"}
            </span>
            ...
            <button
              onClick={manualPing}
              disabled={pinging}
              title="Manually ping the server to wake it up"
              className={`
            ...
            >
              {pinging ? (
                <>
                  <FaSpinner className="animate-spin text-amber-500" /> Pinging…
                </>
              ) : pingResult === "success" ? (
                <>✅ Online</>
              ) : pingResult === "fail" ? (
      ...
      {/* FORM */}
      <div
        className={`${card} border rounded-3xl p-4 ${isOffline ? "opacity-60 pointer-events-none" : ""}`}
      >
        {isOffline && (
          <p className="text-xs text-amber-500 mb-3 flex items-center gap-1">
            <FaSpinner className="animate-spin" /> Form disabled while server
            wakes up…
          </p>
        )}
      ...
      {serverStatus === "online" && (
        <div className="grid grid-cols-3 gap-4">
          {[
            {
              label: "Total Input",
              value: totalInput.toFixed(3),
              color: "text-blue-500",
            },
            {
              label: "Total Output",
              value: totalOutput.toFixed(3),
            ...
            <div
              key={s.label}
              className={`${card} border rounded-2xl p-4 text-center`}
            >
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-12 gap-3 text-slate-400">
            ...
            <p className="text-xs text-slate-400">
              Render free tier needs 30–60 seconds to start
            </p>
          </div>
        ) : serverStatus === "error" ? (
          <div className="flex flex-col items-center justify-center py-12 gap-3 text-red-400">
            ...
            <button
              onClick={manualPing}
              disabled={pinging}
              className="text-xs bg-red-500 text-white px-4 py-2 rounded-xl flex items-center gap-1.5 disabled:opacity-50"
            >
              {pinging ? (
                <>
                  <FaSpinner className="animate-spin" /> Pinging…
                </>
              ) : (
                <>
                  <FaWifi /> Retry Ping
          ...
          <div className="overflow-x-auto rounded-2xl border border-slate-200">
            <table className="w-full min-w-[700px] text-sm">
              ...
              >
                <tr>
                  {["Date", "Carat", "Input", "Output", "Ghat", "Action"].map(
                    (h) => (
                      <th
                        key={h}
                        className={`py-3 px-4 ${h === "Action" ? "text-center" : "text-left"}`}
                      >
                        {h}
                      </th>
                    ),
              ...
              <tbody>
                {filteredRecords.length === 0 ? (
                  ...
                  filteredRecords.map((r, i) => (
                    <tr
                      key={r.id}
                      className={`border-b row-animate ${darkMode ? "border-slate-700 hover:bg-slate-700/40" : "border-slate-200 hover:bg-slate-50"}`}
                      style={{
                        animationDelay: `${i * 0.07}s`,
                        opacity: 0, // starts hidden, animation brings it to 1
                      }}
                    >
              ...
              <tfoot>
                <tr
                  className={`font-bold text-sm ${darkMode ? "bg-slate-700" : "bg-slate-100"}`}
                >
